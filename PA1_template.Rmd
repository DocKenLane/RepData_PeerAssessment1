...
The mean number of steps taken per day is **`r floor(mean(daily$x))`**.
The median number of steps taken per day is **`r median(daily$x)`**.

## What is the average daily activity pattern?
We compute the average number of steps for each 5-minute time interval for
the **`r length(unique(df$date))`** dates in the dataset.
...
The maximum average steps per 5-minute period was **`r floor(maxsteps)`** which occurs
at **`r time`** (24 hour clock).

## Imputing missing values
There are **`r sum(is.na(df$steps))`** missing values of the *steps* variable.
...
With imputted data, the mean number of steps taken per day is **`r floor(mean(daily1$x))`**
(versus *`r floor(mean(daily$x))`*).
The median number of steps taken per day does not change (**`r median(daily1$x)`**).
